...
echo ""
info "  1. Open the gateway URL above in your browser"
info "  2. Start a conversation with your AI agent"
info "  3. To retrieve this URL later: hard-shell url"
echo ""
echo -e "${BLUE}── LLM provider setup ──────────────────────${NC}"
echo ""
info "  Supported providers (configure with 'hard-shell apikey'):"
info "    Anthropic (Claude)  |  OpenAI (GPT-4)  |  Google (Gemini)  |  xAI (Grok)"
echo ""
info "  Using a provider not listed above? (Ollama, Azure, custom endpoint)"
info "    1. Add your API key to $INSTALL_DIR/.env:"
info "         OPENAI_API_KEY=your-key-here"
info "    2. For OpenAI-compatible endpoints, also add the base URL:"
info "         OPENAI_BASE_URL=http://your-host:port/v1"
info "    3. Set the model in $INSTALL_DIR/data/openclaw/openclaw.json:"
info "         \"primary\": \"openai/your-model-name\""
info "    4. Restart: hard-shell restart"
echo ""
echo -e "${BLUE}── Your files ──────────────────────────────${NC}"
echo ""
info "  Config:    $INSTALL_DIR/.env"
info "  Data:      $INSTALL_DIR/data/"
info "  Workspace: $INSTALL_DIR/data/workspace/"
echo ""
echo -e "${BLUE}── Commands (run from anywhere) ─────────────${NC}"
echo ""
